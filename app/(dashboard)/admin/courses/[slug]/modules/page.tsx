...
import { getCourseAdmin } from '@/app/actions/course';
                    ...
                    <div className="space-y-3">
                        {(course.modules || []).length === 0 ? (
                            ...
                            (course.modules || []).sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0)).map((m: any) => (
                                <div key={`${m._id}-${m.order}`} className="border border-gray-100 rounded-xl p-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                        ...
                                        {(m.lessons || []).length === 0 ? (
                                            ...
                                            <ul className="space-y-1">
                                                {(m.lessons || []).sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0)).map((l: any) => (
                                                    <li key={`${l._id}-${l.order}`} className="flex items-center justify-between text-sm text-gray-700">
                                                        <span>
