...
import { getCourseBySlug, updateLessonCompletion, getUserCourseProgress } from '@/app/actions/course';
...
setCourse({
  ...
  thumbnail:
    !result.course.thumbnail
      ? undefined
      : typeof result.course.thumbnail === 'string'
      ? result.course.thumbnail
      : result.course.thumbnail.url,
});          
 ...
 const markLessonComplete = async (lessonId: string) => {
  ...
  // Add null check for currentModule
  if (!user?.id || !course?._id || !currentModule) {
    console.log("Missing currentModule");
    return;
  }
  ...
  try {
    const result = await updateLessonCompletion(
      user.id,
      course._id,
      currentModule._id,  // Now currentModule is guaranteed to be non-null
      lessonId,
      newCompleted
    );
    
    if (result.success) {
      console.log("Lesson completion updated successfully");
    ...
    // On error, revert the optimistic update
    startTransition(() => {
    if (optimisticCompleted.has(lessonId) !== wasCompleted) {
      setOptimisticCompleted(lessonId);
    }
  });
  ...
  const onPlayerStateChange: YouTubeProps['onStateChange'] = async (event) => {
    ...
    if (event.data === 0 && currentLesson && !completedLessons.has(currentLesson._id)) {
      console.log("Completed",currentLesson);
