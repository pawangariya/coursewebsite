...
import { uploadBase64ImageToCloudinary } from '@/app/utils/cloudinary';
...
export async function getCourseBySlug(slug: string) {
  try {
    ...
    const formattedCourse = {
      ...course.toObject(),
      _id: course._id.toString(), // top-level course _id
...
export async function checkSubscription(clerkId: string) {
  try {
    ...
    return {
      success: true,
      isSubscribed: true, // Boolean: does a completed subscription exist?
      subscription: null
    };
  } catch (error) {
...
export async function updateLessonCompletion(
  userId: string,
  courseId: string,
  moduleId: string,  // Add moduleId parameter
  lessonId: string,
  completed: boolean
) {
  try {
    ...
    if (!userProgress) {
      // First, get the course to know the structure
      const course = await Course.findById(courseId);
      if (!course) {
        return { success: false, error: 'Course not found' };
      }
    ...
    // Find the module
    let moduleIndex = userProgress.modules.findIndex(
      (m: any) => m.moduleId.toString() === moduleId
    );
    
    // If module not found, we add it (this might happen if the course structure changed)
    if (moduleIndex === -1) {
      // We need to get the course to get the module and its lessons
      const course = await Course.findById(courseId);
      if (!course) {
        return { success: false, error: 'Course not found' };
      }
      
      const courseModule = course.modules.find((m: any) => m._id.toString() === moduleId);
      if (!courseModule) {
        return { success: false, error: 'Module not found in course' };
      }
      
      // Create a new module progress entry
      const newModule = {
        moduleId: moduleId,
        completed: false,
        progress: 0,
      ...
      userProgress.modules.push(newModule);
      moduleIndex = userProgress.modules.length - 1;
    }
    
    // Find the lesson in the module
    let lessonIndex = userProgress.modules[moduleIndex].lessons.findIndex(
      (l: any) => l.lessonId.toString() === lessonId
    );
    
    // If lesson not found, add it
    if (lessonIndex === -1) {
      ...
      lessonIndex = userProgress.modules[moduleIndex].lessons.length - 1;
    } else {
      // Update existing lesson
      userProgress.modules[moduleIndex].lessons[lessonIndex].completed = completed;
      userProgress.modules[moduleIndex].lessons[lessonIndex].completedAt = 
        completed ? new Date() : undefined;
    }
    
    // Update module progress
    const totalLessons = userProgress.modules[moduleIndex].lessons.length;
    const completedLessons = userProgress.modules[moduleIndex].lessons.filter((l: any) => l.completed).length;
    userProgress.modules[moduleIndex].progress = totalLessons > 0 ? Math.round((completedLessons / totalLessons) * 100) : 0;
    
    // Check if module is completed
    if (userProgress.modules[moduleIndex].progress === 100) {
      userProgress.modules[moduleIndex].completed = true;
      userProgress.modules[moduleIndex].completedAt = new Date();
    } else {
      userProgress.modules[moduleIndex].completed = false;
      userProgress.modules[moduleIndex].completedAt = undefined;
    }
    
    // Update overall progress
    let totalCourseLessons = 0;
    let totalCompletedLessons = 0;
    userProgress.modules.forEach((mod: any) => {
      totalCourseLessons += mod.lessons.length;
      totalCompletedLessons += mod.lessons.filter((l: any) => l.completed).length;
    });
    
    userProgress.overallProgress = totalCourseLessons > 0 ? Math.round((totalCompletedLessons / totalCourseLessons) * 100) : 0;
